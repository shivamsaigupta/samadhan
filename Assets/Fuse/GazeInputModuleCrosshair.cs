...
[RequireComponent(typeof(GazeInputModuleFuse))]
public class GazeInputModuleCrosshair : MonoBehaviour 
{
	...
	private GazeInputModuleFuse gazeInputModuleFuse;
	private Vector3 CrosshairOriginalScale;
	private float CrosshairOriginalDistance;

	void Awake()
	{
		gazeInputModuleFuse = GetComponent<GazeInputModuleFuse>();
	}
	...
	void Update () 
	{
		...
		if (DisplayCrosshair)
		{
			...
			if (gazeInputModuleFuse.CurrentRaycast.isValid)
			{
				distance = gazeInputModuleFuse.CurrentRaycast.distance * 0.8f - 0.5f;
			}
